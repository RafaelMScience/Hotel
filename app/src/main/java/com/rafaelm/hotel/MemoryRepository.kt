...
object MemoryRepository : HotelRepository{
    ...
    init{
        ...
        save(Hotel(0,"Cenario Hotel", "Rua dos Navegantes", 3.0f))
        ...
        save(Hotel(0,"Hotel Infite", "Rua Ribeiro de Brito", 5.0f))
    ...
    override fun search(term: String, callback: (List<Hotel>) -> Unit) {
        callback(
            if (term.isEmpty()) hotelsList
                ...
                it.name.toUpperCase(Locale.getDefault()).contains(term.toUpperCase(Locale.getDefault()))
            }
        )
    }
}